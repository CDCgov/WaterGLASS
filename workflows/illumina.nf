/*
 This pipeline is a fork of nf-core/viralrecon and uses 95% of that pipeline's plumbing,
 plus some finetuning and modifications for microbial eukaryotic genomes.
 All proper credit and thanks are given to the original nf-core authors.
*/
...
def valid_params = [
    protocols         : ['metagenomic', 'isolate', 'amplicon'],
    variant_callers   : ['ivar', 'bcftools'],
    consensus_callers : ['ivar', 'bcftools'],
    assemblers        : ['spades', 'unicycler', 'skesa', 'minia', 'flye', 'dragonflye'],
...
// Classification databases setup --> activates those tools for Cryptosporidium if these are provided 
if ( params.db_18S ) {
    ch_db_18S = Channel
        .value(file( "${params.db_18S}" ))
} else {
    ch_db_18S = Channel.empty()
}
if ( params.db_gp60 ) {
    ch_db_gp60 = Channel
        .value(file( "${params.db_gp60}" ))
} else {
    ch_db_gp60 = Channel.empty()
}
...
include { CRYPTO_SSU_CLASSIFICATION as CRYPTO_SPECIES_IDENTIFICATION } from '../modules/local/ssu_classification'
include { CAT_18S                                                    } from '../modules/local/cat_18S'
include { CRYPTO_GP60_SUBTYPING                                      } from '../modules/local/gp60_subtyping'
include { CAT_GP60                                                   } from '../modules/local/cat_gp60'

include { CRYPTO_SSU_CLASSIFICATION_skesa as CRYPTO_SPECIES_IDENTIFICATION_skesa } from '../modules/local/ssu_classification_skesa'
include { CAT_18S_skesa                                                    } from '../modules/local/cat_18S_skesa'
include { CRYPTO_GP60_SUBTYPING_skesa                                      } from '../modules/local/gp60_subtyping_skesa'
include { CAT_GP60_skesa                                                   } from '../modules/local/cat_gp60_skesa'
include { FASTANI_PREP                                               } from '../modules/local/fastani_prep'   
include { FASTANI_PREP_SKESA                                         } from '../modules/local/fastani_prep_skesa'
include { FASTANI_SKESA                                              } from '../modules/local/fastani_skesa'      
...
include { ETOKI_MLST_SKESA                                           } from '../modules/local/etoki_skesa'
include { ETOKI_MLST_UNICYCLER                                       } from '../modules/local/etoki_unicycler'
...
include { ASSEMBLY_SKESA          } from '../subworkflows/local/assembly_skesa'
include { BBMAP_ALIGN             } from '../subworkflows/local/bbmap'
...
workflow ILLUMINA {
    ...
    if (!params.skip_kraken2) {
        ...
        if (params.kraken2_variants_host_filter) {
            ch_variants_fastq = KRAKEN2_KRAKEN2.out.classified_reads_fastq
        }

        if (params.kraken2_assembly_host_filter) {
            ch_assembly_fastq = KRAKEN2_KRAKEN2.out.classified_reads_fastq
        }
    }
    ...
    // SUBWORKFLOW: Run Skesa assembly and downstream analysis
    ch_skesa_quast_multiqc = Channel.empty()
    if (!params.skip_assembly && 'skesa' in assemblers) {
        ASSEMBLY_SKESA (
            ch_assembly_fastq.map { meta, fastq -> [meta, fastq, [] ]},
            PREPARE_GENOME.out.fasta,
            params.gff ? PREPARE_GENOME.out.gff : [],
        ...
        ch_skesa_quast_multiqc = ASSEMBLY_SKESA.out.quast_tsv
        ch_versions            = ch_versions.mix(ASSEMBLY_SKESA.out.versions)
    }

    // SUBWORKFLOW: Run Flye assembly and downstream analysis

    // SUBWORKFLOW: Run Dragonflye assembly and downstream analysis

    // SUBWORKFLOW: Run bbmap to align fastq reads to assemblies genome (skesa and unicycler) and reference genome
    ch_bbmap_multiqc = Channel.empty()
    ch_skesa_assemblies =  ASSEMBLY_SKESA.out.skesa_scaffolds
    ch_assemblies = ASSEMBLY_UNICYCLER.out.scaffolds
    ch_bbmap_bam = Channel.empty()
    BBMAP_ALIGN( ch_assembly_fastq, params.fasta, ch_assemblies, ch_skesa_assemblies)



    // Custom tool analysis: 18S-based species identification and GP60 subtyping\
    if (params.db_18S ) {
        CRYPTO_SPECIES_IDENTIFICATION (
            ch_assemblies,
            ch_db_18S
        )
        ch_18S_list = CRYPTO_SPECIES_IDENTIFICATION.out.blast_results.collect{it[1]}
        CAT_18S (
            ch_18S_list
        )
    }
    if (params.db_gp60 ) {
        CRYPTO_GP60_SUBTYPING (
            ch_assemblies,
            ch_db_gp60
        )
        ch_gp60_list = CRYPTO_GP60_SUBTYPING.out.gp60.collect{it[1]}
        CAT_GP60 (
            ch_gp60_list
        )
    }
     ...
     if (params.db_18S ) {
        CRYPTO_SPECIES_IDENTIFICATION_skesa (
            ch_skesa_assemblies,
            ch_db_18S
        )
        ch_18S_list_skesa = CRYPTO_SPECIES_IDENTIFICATION_skesa.out.blast_results_skesa.collect{it[1]}
        CAT_18S_skesa (
            ch_18S_list_skesa
        )
    }
    if (params.db_gp60 ) {
        CRYPTO_GP60_SUBTYPING_skesa (
            ch_skesa_assemblies,
            ch_db_gp60
        )
        ch_gp60_list_skesa = CRYPTO_GP60_SUBTYPING_skesa.out.gp60_skesa.collect{it[1]}
        CAT_GP60_skesa (
            ch_gp60_list_skesa
        )
    }
    ...
    //SUBWORKFLOW: to prepare the allele database to run etoki
    PREPARE_ALLELEDB(params.allele_seq)
    
     //MODULE:LOCAL to run the MLST calling
    ETOKI_MLST_SKESA(ch_skesa_assemblies,PREPARE_ALLELEDB.out.reference_alleles,PREPARE_ALLELEDB.out.etoki_md5sum )
    ETOKI_MLST_UNICYCLER(ch_assemblies,PREPARE_ALLELEDB.out.reference_alleles,PREPARE_ALLELEDB.out.etoki_md5sum )


    //FASTANI
    FASTANI_PREP(ASSEMBLY_UNICYCLER.out.scaffolds.collect{it[1]})
    FASTANI(FASTANI_PREP.out.fastani_prepfile_unicycler)

    FASTANI_PREP_SKESA(ASSEMBLY_SKESA.out.skesa_scaffolds.collect{it[1]})
    FASTANI_SKESA(FASTANI_PREP_SKESA.out.fastani_prepfile_skesa)

    KMA_ALLELES("${params.allele_seq}")
    KMACGMLST_INDEX(KMA_ALLELES.out.fasta)
    ch_schema_kma = KMACGMLST_INDEX.out.index
    KMACGMLST_CALL(ch_assembly_fastq,ch_schema_kma)
    ...
    if (!params.skip_multiqc) {
        ...
        MULTIQC (
            ...
            ch_skesa_quast_multiqc.collect().ifEmpty([]),
            CAT_18S.out.ssu18S_results.collectFile(name: 'results_18S_mqc.csv').ifEmpty([]),
            CAT_GP60.out.gp60_results.collectFile(name: 'results_gp60_mqc.tsv').ifEmpty([])
        )
