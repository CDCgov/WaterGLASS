//
// Assembly and downstream processing for Flye (long read) scaffolds
//
...
workflow ASSEMBLY_DRAGONFLYE {
    ...
    // Assemble reads with Dragonflye
    DRAGONFLYE (
        ch_reads,
    )
    ...
    emit:
    scaffolds          = DRAGONFLYE.out.contigs             // channel: [ val(meta), [ scaffolds ] ]
    raw_contigs        = DRAGONFLYE.out.raw_contigs         // channel: [ val(meta), [ scaffolds ] ]
    gv                 = DRAGONFLYE.out.gv                  // channel: [ val(meta), [ gv ] ]
    txt                = DRAGONFLYE.out.txt                 // channel: [ val(meta), [ dragonflye_txt ] ]
    gfa                = DRAGONFLYE.out.gfa                 // channel: [ val(meta), [ gfa ] ]
    log_out            = DRAGONFLYE.out.log                 // channel: [ val(meta), [ log ] ]
