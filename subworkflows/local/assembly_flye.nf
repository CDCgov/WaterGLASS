...
workflow ASSEMBLY_FLYE {
    take:
    reads        // channel: [ val(meta), [ reads ] ]
    mode         // string : Flye assembly mode e.g. 'nano-hq'
    ...
    emit:
    scaffolds          = FLYE.out.fasta                     // channel: [ val(meta), [ scaffolds ] ]
    gv                 = FLYE.out.gv                        // channel: [ val(meta), [ gv ] ]
    txt                = FLYE.out.txt                       // channel: [ val(meta), [ flye_txt ] ]
    gfa                = FLYE.out.gfa                       // channel: [ val(meta), [ gfa ] ]
    log_out            = FLYE.out.log                       // channel: [ val(meta), [ log ] ]
